...
## Figures 
The script for generating figures is `results.ipynb` located in `./results`. The `.txt` benchmark files generated by the **presto-benchmark-driver** are located in the same directory. The generated figures are included in the jupyter notebook mentioned.
